...
-------------------------------------
INSERT INTO PI_INGEST.DIM_CUSTOMER
...
-------------------------------------
INSERT INTO PI_INGEST.DIM_PRODUCT (PRODUCT_ID, PRODUCT_NAME, PRODUCT_DESC, CATEGORY, PRICE)
VALUES
...
INSERT INTO PI_INGEST.DIM_PRODUCT
...
-------------------------------------
INSERT INTO PI_INGEST.FACT_ORDER (ORDER_ID, CUSTOMER_SK, ORDER_DATE, ORDER_STATUS, TOTAL_AMOUNT, CURRENCY)
VALUES(SELECT 'O5001', CUSTOMER_SK, CURRENT_TIMESTAMP(), 'Completed', 78000, 'INR'
FROM PI_INGEST.DIM_CUSTOMER WHERE CUSTOMER_ID = 1001 AND IS_ACTIVE = 'Y');

INSERT INTO PI_INGEST.FACT_ORDER (ORDER_ID, CUSTOMER_SK, ORDER_DATE, ORDER_STATUS, TOTAL_AMOUNT, CURRENCY)
VALUES(SELECT 'O5002', CUSTOMER_SK, CURRENT_TIMESTAMP(), 'Pending', 16000, 'INR'
FROM PI_INGEST.DIM_CUSTOMER WHERE CUSTOMER_ID = 1002 AND IS_ACTIVE = 'Y');

-------------------------------------
-- 5) FACT_ORDER_ITEM (joins product + order)
-------------------------------------
-- Order O5001
INSERT INTO PI_INGEST.FACT_ORDER_ITEM (ORDER_SK, PRODUCT_SK, QUANTITY, UNIT_PRICE, LINE_TOTAL)
  VALUES(
SELECT
  f.ORDER_SK,
  p.PRODUCT_SK,
  ...
  p.PRICE,
  p.PRICE
FROM PI_INGEST.FACT_ORDER f
JOIN PI_INGEST.DIM_PRODUCT p ON p.PRODUCT_ID = 'P101' AND p.IS_ACTIVE = 'Y'
...
SELECT
  ...
  4 * p.PRICE
FROM PI_INGEST.FACT_ORDER f
JOIN PI_INGEST.DIM_PRODUCT p ON p.PRODUCT_ID = 'P103' AND p.IS_ACTIVE = 'Y'
WHERE f.ORDER_ID = 'O5002');

-------------------------------------
-- 6) FACT_PAYMENT
-------------------------------------
INSERT INTO PI_INGEST.FACT_PAYMENT (PAYMENT_ID, ORDER_SK, PAYMENT_DATE, PAYMENT_METHOD, AMOUNT, CURRENCY, PAYMENT_STATUS)
  VALUES(
SELECT
  'PAY9001', ORDER_SK, CURRENT_TIMESTAMP(), 'Card', 78000, 'INR', 'Success'
FROM PI_INGEST.FACT_ORDER WHERE ORDER_ID = 'O5001');

INSERT INTO PI_INGEST.FACT_PAYMENT (PAYMENT_ID, ORDER_SK, PAYMENT_DATE, PAYMENT_METHOD, AMOUNT, CURRENCY, PAYMENT_STATUS)
  VALUES(
SELECT
  'PAY9002', ORDER_SK, CURRENT_TIMESTAMP(), 'UPI', 16000, 'INR', 'Pending'
FROM PI_INGEST.FACT_ORDER WHERE ORDER_ID = 'O5002');

-------------------------------------
-- 7) RAW_ORDERS
-------------------------------------
INSERT INTO PI_INGEST.RAW_ORDERS (RAW_ORDER_ID, ORDER_JSON)
VALUES
...
-- 8) LOAD_AUDIT_LOG
-------------------------------------
INSERT INTO PI_INGEST.LOAD_AUDIT_LOG
